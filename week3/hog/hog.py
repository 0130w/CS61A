...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            # Player 0's turn
            num = strategy0(score0, score1) # Get the number of dices
            score0 = update(num, score0, score1, dice)
        else:
            # Player 1's turn
            num = strategy1(score1, score0) # Get the number of dices
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while(start <= 10):
        test_dice = dice
        value = make_averaged(roll_dice, total_samples)(start, test_dice)
        if value > maxn:
            maxn = value
            pos = start
        start += 1
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    value = tail_points(opponent_score)
    if value >= threshold:
        return 0
    return num_rolls
    # END PROBLEM 10
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    init_score = score
    score = square_update(0, score, opponent_score)
    if score - init_score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** Just A naive combination QwQ , but get a little better result ***
    """
    # BEGIN PROBLEM 12
    if score <= 80:
        return square_strategy(score, opponent_score, 12, 6)
    else:
        return square_strategy(score, opponent_score, 6, 2)
    # END PROBLEM 12
